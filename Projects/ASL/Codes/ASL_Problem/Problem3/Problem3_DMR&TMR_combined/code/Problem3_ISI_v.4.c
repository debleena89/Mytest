...
void verif_check_dips( int verift, unsigned char verifdips[], __CPROVER_bool onoff )
{
	...
	for (i = 0, j = 1; i < verifdips[0]; i++, j = j + 2)
	{
		...
		get_dmr_value = get_dmr_status(verifdips[j + 1], onoff);
               ...
               /* P = (verifdips[j] == 0);
                Q = (lookup_status[0x00][verifdips[j + 1]].unit == INVALID);  
                R = (lookup_status[0x00][verifdips[j + 1]].common == INVALID);
                S = (onoff == get_tmr_status(verifdips[j + 1],  lookup_status[0x00][verifdips[j + 1]].common));
                T = (survstatbuf[12 * verifdips[j] + verifbyte + 4] & verifmask);*/
	        ...
	        NOK[2] = (verifdips[j] == 0) && !(lookup_status[0x00][verifdips[j + 1]].unit == INVALID) && 
                ...
                OK[3] = !(verifdips[j] == 0) && (survstatbuf[12 * verifdips[j] + verifbyte + 4] & verifmask) && onoff;

                NOK[3] = !(verifdips[j] == 0) && !(survstatbuf[12 * verifdips[j] + verifbyte + 4] & verifmask) && onoff;


                OK[4] = !(verifdips[j] == 0) && !(survstatbuf[12 * verifdips[j] + verifbyte + 4] & verifmask) && !onoff;  		

                NOK[4] = !(verifdips[j] == 0) && (survstatbuf[12 * verifdips[j] + verifbyte + 4] & verifmask) && !onoff;
