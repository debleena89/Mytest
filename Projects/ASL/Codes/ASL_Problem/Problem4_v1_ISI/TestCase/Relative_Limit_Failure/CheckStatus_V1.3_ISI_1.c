/***Bug is at line no.-90,  assertions for Relative Limit Failed***/
/*****************************************************************************/
	...
	for(i=0; i<3; i++)
   	{
     	...
     	{
			Interstatus = Interstatus | (0x0000 << (i+4) );//bug
...
void VerifCheckStatus(int cycle)
{
   ...
   /********* asserts for relative limit failure conditions represented in bit4(X), bit5(Y), bit6(Z) of Interstatus word *********/
	__CPROVER_assert(!RelativeDataFailure_X || ((Interstatus & 0x0010) == 0x0010), "Relative X Fails"); // All the
	__CPROVER_assert(!RelativeDataFailure_Y || ((Interstatus & 0x0020) == 0x0020), "Relative Y Fails"); //assertions 
	__CPROVER_assert(!RelativeDataFailure_Z || ((Interstatus & 0x0040) == 0x0040), "Relative Z Fails"); // are Failed
