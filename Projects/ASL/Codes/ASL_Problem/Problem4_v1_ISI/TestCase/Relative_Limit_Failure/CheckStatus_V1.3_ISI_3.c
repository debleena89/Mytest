/***Bug is at line no.-88,  assertions for Relative Limit Failed***/
/*****************************************************************************/
...
void  CheckStatus()
{
	...
	for(i=0; i<3; i++)
   	{
     	if(fabs(Sys1Count[i] - Sys1Count_old[i]) < 0.0489) // for jerk //bug
     	{
...
void VerifCheckStatus(int cycle)
{
	...
	RelativeDataFailure_X = (((fabs(Sys1Count[0] - Sys1Count_old[0])) > 0.0489) && DataCheckModeValidity);
	RelativeDataFailure_Y = (((fabs(Sys1Count[1] - Sys1Count_old[1])) > 0.0489) && DataCheckModeValidity);
	RelativeDataFailure_Z = (((fabs(Sys1Count[2] - Sys1Count_old[2])) > 0.0489) && DataCheckModeValidity);

   /********* asserts for relative limit failure conditions represented in bit4(X), bit5(Y), bit6(Z) of Interstatus word *********/
	__CPROVER_assert(!RelativeDataFailure_X || ((Interstatus & 0x0010) == 0x0010), "Relative X Fails"); // All
	__CPROVER_assert(!RelativeDataFailure_Y || ((Interstatus & 0x0020) == 0x0020), "Relative Y Fails"); // assertions
	__CPROVER_assert(!RelativeDataFailure_Z || ((Interstatus & 0x0040) == 0x0040), "Relative Z Fails"); // Failed
