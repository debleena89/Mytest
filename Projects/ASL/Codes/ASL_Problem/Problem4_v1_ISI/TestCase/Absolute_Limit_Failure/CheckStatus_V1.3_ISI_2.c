/***Bug is at line no.-107, All assertions did not Fail***/
/*****************************************************************************/
...
void  CheckStatus()
{
	...
	if( ((Sys1Count[i] > 0.625) || (Sys1Count[i] = -0.625)) && ((mode_word == 11) || (mode_word == 44)) ) // Bug inserted
      	{
...
void VerifCheckStatus(int cycle)
{
	...
	AbsoluteDataIntermittentFailure_X[cycle] = ((Sys1Count[0] > 0.625 || Sys1Count[0] < -0.625) && (Sys1Count[0] != -0.625) 
                                                    && DataCheckModeValidity);
	AbsoluteDataIntermittentFailure_Y[cycle] = ((Sys1Count[1] > 0.625 || Sys1Count[1] < -0.625) && (Sys1Count[1] != -0.625) 
                                                    && DataCheckModeValidity);
	AbsoluteDataIntermittentFailure_Z[cycle] = ((Sys1Count[2] > 0.625 || Sys1Count[2] < -0.625) && (Sys1Count[2] != -0.625)
                                                    && DataCheckModeValidity);
