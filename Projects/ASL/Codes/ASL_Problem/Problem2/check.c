 ...
 void verifDc_Dc(int verifchno)
{
    ...
    __CPROVER_bool OKC[8];

    OKC[0] = (verifchno < 96) && (memcmp(zero_array, survstatbuf, 12) == 0);
    OKC[1] = (verifchno < 96) && !(memcmp(zero_array, survstatbuf, 12) == 0);

    OKC[2] = (verifchno < 96) && (memcmp(zero_array, survstatbuf2, 12) == 0);
    OKC[3] = (verifchno < 96) && !(memcmp(zero_array, survstatbuf2, 12) == 0);

    OKC[4] = (verifchno >= 96) && (memcmp(&zero_array[12], &survstatbuf[12], 4) == 0);
    OKC[5] = (verifchno >= 96) && !(memcmp(&zero_array[12], &survstatbuf[12], 4) == 0);

    OKC[6] = (verifchno >= 96) && (memcmp(&zero_array[12], &survstatbuf2[12], 4) == 0);
    OKC[7] = (verifchno >= 96) && !(memcmp(&zero_array[12], &survstatbuf2[12], 4) == 0);
                                     
   /* Assume on chnum -- channel number can be between 0 and 127 only */
    __CPROVER_assume(verifchno >= 0 && verifchno <= 127);

    __CPROVER_assert (!OKC[0] || (MainDC_DC == 0), "Hey OKC0 fails"); 
    __CPROVER_assert (!OKC[1] || (MainDC_DC == 1), "Hey OKC1 fails");
    __CPROVER_assert (!OKC[2] || (StdbyDC_DC == 0), "Hey OKC2 fails");
    __CPROVER_assert (!OKC[3] || (StdbyDC_DC == 1), "Hey OKC3 fails");
    __CPROVER_assert (!OKC[4] || (MainDC_DC == 0), "Hey OKC4 fails");
    __CPROVER_assert (!OKC[5] || (MainDC_DC == 1), "Hey OKC5 fails");
    __CPROVER_assert (!OKC[6] || (StdbyDC_DC == 0), "Hey OKC6 fails");
    __CPROVER_assert (!OKC[7] || (StdbyDC_DC == 1), "Hey OKC7 fails");
}
...
int main()
{
    ...
    verifDc_Dc (chno); // calling the verif function for checkDC_DC

    verif_get_tmr( t ); // calling the verif function for get_tmr_status
}
