/* Negative Test Case 7, OK[0],OK[2],OK[4],NOK[1],NOK[3]and NOK[5] fails */
...
bool get_dmr_status(unsigned char chnum, bool on_off)
{
    ...
    /////////////detect output///////////////////////
    if((!No_liu1_data && !liu1_notok) ||(!No_liu2_data && !liu2_notok))//Bug introduced here for double error
    {
        if((survstatbuf[byte] & mask) && (survstatbuf2[byte]&mask)) // Bug introduced here
            output = true;
        else
            output = false;
    }
