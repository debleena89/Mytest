/* Negative Test Case 8, all cases where SYS1NOK is there should fail */
...
bool get_dmr_status(unsigned char chnum, bool on_off)
{
    ...
    {
        if((survstatbuf[byte] & mask) && (survstatbuf2[byte]&mask))//bug for double error
            output = true;
        else
            output = false;
    }
    else if (!No_liu1_data || !liu1_notok) /* Bug Inserted */
