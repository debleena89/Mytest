/* Negative Test Case 3, expect OK[4],OK[5],NOK[4],NOK[5],NOK[6] to fail */
...
bool get_dmr_status(unsigned char chnum, bool on_off)
{
    ...
    else if (!No_liu1_data || !liu1_notok)//Bug introduced here for double error
        output = survstatbuf[byte] & mask;
    else if(!No_liu2_data && !liu2_notok)
        output = survstatbuf2[byte] & mask;
    else 
        output_undef = false; // Bug introduced here
